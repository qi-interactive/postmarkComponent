...
class Postmark extends CComponent {

    public $postmarkAppApiKey = null;
    public $mailFromName = "postmark@qi-interactive.com";
    public $mailFromEmailAdddress = "devnotification@qi-interactive.com";

    public $devNotificationEmailAddress = "devnotification@qi-interactive.com";
    ...
    public function __construct() {
        $this->_apiKey = $this->postmarkAppApiKey;

        $this->from($this->mailFromEmailAdddress);
    ...
    public function sendDevNotification($subject, $message, $tags = array()) {
        $email = new Postmark();
        $email->addTo($this->mailFromEmailAdddress)
                ->subject($subject)
    ...
    public function &addAttachment($filename, $options = array()) {
        ...
        $this->addCustomAttachment(
                isset($options['filenameAlias']) ? $options['filenameAlias'] : basename($filename), file_get_contents($filename), $this->_getMimeType($filename)
        );
